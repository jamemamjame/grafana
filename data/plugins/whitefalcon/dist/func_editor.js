System.register(['angular', 'lodash', 'jquery'], function(exports_1) {
    var angular_1, lodash_1, jquery_1;
    var WhitefalconFuncEditor;
    return {
        setters:[
        ...
        execute: function() {
            WhitefalconFuncEditor = (function () {
                function WhitefalconFuncEditor() {
                    angular_1.default
                        ...
                        var funcControlsTemplate = '<div class="tight-form-func-controls">' +
                        ...
                        return {
                            ...
                            link: function postLink($scope, elem) {
                                var $funcLink = jquery_1.default(funcSpanTemplate);
                                var $funcControls = jquery_1.default(funcControlsTemplate);
                                var ctrl = $scope.ctrl;
                                var func = $scope.func;
                                    ...
                                    if (options === "_tags_") {
                                        options = function (query, process) {
                                            ctrl.datasource.queryTags(query, ctrl.getSegmentPathUpTo())
                                                ...
                                                var tvars = lodash_1.default.map(templateSrv.variables, function (v) { return '$' + v.name; });
                                ...
                                function addElementsAndCompile() {
                                    ...
                                    lodash_1.default.each(funcDef.params, function (param, index) {
                                        if (param.optional && func.params.length <= index) {
                                            return;
                                        }
                                        if (index > 0) {
                                            jquery_1.default('<span>, </span>').appendTo(elem);
                                        }
                                        var paramValue = templateSrv.highlightVariablesAsHtml(func.params[index]);
                                        var $input = jquery_1.default(paramTemplate);
                                        paramCountAtLink++;
                                        if (funcDef.params[index].type !== "csv") {
                                            var $paramLink = jquery_1.default('<a ng-click="" class="whitefalcon-func-param-link">' + paramValue + '</a>');
                                ...
                                function registerFuncControlsActions() {
                                    $funcControls.click(function (e) {
                                        var $target = jquery_1.default(e.target);
                                        ...
                                        if ($target.hasClass('fa-arrow-left')) {
                                            $scope.$apply(function () {
                                                lodash_1.default.move(ctrl.functions, $scope.$index, $scope.$index - 1);
                                                ctrl.targetChanged();
                                            });
                                            return;
                                        }
                                        ...
                                        if ($target.hasClass('fa-question-circle')) {
                                            window.open("http://whitefalcon.readthedocs.org/en/latest/" +
                                                "functions.html#whitefalcon.render.functions." + funcDef.name, '_blank');
                                            return;
                                        }
                                    });
                                }
                ...
                return WhitefalconFuncEditor;
            })();
            exports_1("default",WhitefalconFuncEditor);
        }
    }
});
